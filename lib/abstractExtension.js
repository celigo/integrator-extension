...
AbstractExtension.prototype.loadConfiguration = function (config, callback) {
  ...
  validateConfiguration.call(this, config, function (err) {
    ...
    logger.info('Loaded integrator-extension configuration.')
    if (self.configuration.diy) logger.info('Loaded DIY module.')
    if (self.configuration.connectors) {
      _.forEach(self.configuration.connectors, function (value, _connectorId) {
